...
exports.unreadmessagecount = async function(req, res) {
  ...
  try {
    ...
    if(req.user.login_as == 'cook'){
      cook_id = req.user.user_id;
      var messageData = await Message.countDocuments({cook_id: cook_id, from: 'user'})
    }
    if(req.user.login_as == 'user'){
      user_id = req.user.user_id;
      var messageData = await Message.countDocuments({user_id: user_id, from: 'cook'})
    }
...
exports.allchatusers = async function(req, res) {
  try {
    if(req.user.login_as == 'cook'){
      const allUsers = await Message.find({cook_id: req.user.user_id}).select('user_id').distinct('user_id');
      const allUsersData = await User.find({_id:  {$in: allUsers}})
                          .populate({path: 'unreadCount',match: { is_read: false, from: 'user' } })
                          .populate({path: 'lastMessage', options: { sort: { 'created_at': -1 }, perDocumentLimit: 1} })
                          .lean()
                          .exec();
      return res.status(201).json({"status": true, 'data': allUsersData});
    } else {
      const allUsers = await Message.find({user_id: req.user.user_id}).select('cook_id').distinct('cook_id');
      const allUsersData = await Cook.find({_id:  {$in: allUsers}})
