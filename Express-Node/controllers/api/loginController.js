...
exports.refresh = async function(req, res) {
  try {
      ...
      var login_as = 'cook';
      var user = await Cook.findOne({ _id:  req.user.user_id}).lean().exec();  
      if(!user){
        login_as = 'user';
        var user = await User.findOne({ _id:  req.user.user_id}).lean().exec();  
      }

      if(user){
        const token = jwt.sign(
          { user_id: user._id, email:user.email, login_as: login_as },
