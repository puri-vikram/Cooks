...
exports.add = async function(req, res) {
  ...
  try {
    const { title, communication, presentation, taste, punctuality, cleanliness, value,description, cook_id } = req.body;
    if (!(title && communication && presentation && taste && punctuality && cleanliness && value && description && cook_id)) {
      return res.status(400).send({"status": false,'message': "All input is required"});
    }
    var rate = ((communication + presentation + taste + punctuality + cleanliness + value)/6);
...
exports.edit = async function(req, res) {
  ...
  try {
    ...
    if(ReviewData){
      ...
      return res.status(201).json({"status": true, 'data': newReviewData, 'message': "updated Successfully"});
    } else {
